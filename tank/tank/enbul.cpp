...
#include "map.h"
using namespace sf;
	Enemy_Bul::Enemy_Bul(Image &image, float X, float Y, int W, int H, std::string  Name, int dir) :Entity(image, X, Y, W, H, Name){
		x = X;
		y = Y;
		direction = dir;
		speed = 0.5;
	...
	void Enemy_Bul::randomBulGenerate(){
srand(time(0));	
}

	void Enemy_Bul::update(float time)
	{
		switch (direction)
		{
